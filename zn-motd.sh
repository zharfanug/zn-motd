...
print_disk_usage() {
  ...
  disks=$(df | grep -vE 'tmpfs|/dev/|/wsl|WSL2' | awk '{print $6}' | tail -n +2 | grep -vE '^(/boot|/snap|/dev|/run|/init)' | grep -vE '^(/)$')
  if [ -n "$disks" ]; then
